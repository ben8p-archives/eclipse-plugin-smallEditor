...
public abstract class AQueuedRuleBasedScanner extends RuleBasedScanner {
	...
	protected static class Entry {
		...
		public Entry(ITokenScanner tokenScanner, IToken token, int offset,
				int length) {
		...
		public String toString() {
			return MessageFormat.format(
					"[{0}, {1}]: {2}", offset, length, token.getData()); //$NON-NLS-1$
		}
	}
	...
	/**
	 * Queue the specified delegate token scanner with offset:length range for
	 * return by nextToken() The given delegate will be used until EOF token
	 * returned.
	 * 
	 * @param tokenScanner
	 * @param offset
	 ...
	 * 
	 * @see
	 * org.eclipse.jface.text.rules.RuleBasedScanner#setRange(org.eclipse.jface
