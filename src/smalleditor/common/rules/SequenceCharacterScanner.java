...
import smalleditor.common.IPartitionScannerSwitchStrategy.ISequenceBypassHandler;
	...
	public int read() {
		...
		if (c != ICharacterScanner.EOF && !ignored) {
			if (switchSequences == null) {
				switchSequences = TextUtility.replace(switchStrategy.getSwitchSequences(), '\n', TextUtility.rsort(characterScanner.getLegalLineDelimiters()));
			}
			for (char[] sequence : switchSequences) {
				if (c == sequence[0] && TextUtility.sequenceDetected(characterScanner, sequence, ignoreCase)) {
